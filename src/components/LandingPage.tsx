...
function ProfitChart() {
	// Generate upward trending data points with more dramatic rise
	const dataPoints = Array.from({ length: 20 }, (_, i) => {
		// More dramatic upward curve
		const baseValue = 40 + Math.pow(i / 2, 1.5)
		const noise = Math.random() * 8 - 4 // Add some realistic fluctuation
	...
	return (
		<div className="w-full h-full relative">
			<svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
				...
				{/* Chart line */}
				<polyline
					points={dataPoints.map((point, i) => `${(i / (dataPoints.length - 1)) * 100},${100 - point}`).join(" ")}
					fill="none"
					stroke="cyan"
					strokeWidth="1"
					opacity="1"
				...
				<polygon
					points={`0,100 ${dataPoints.map((point, i) => `${(i / (dataPoints.length - 1)) * 100},${100 - point}`).join(" ")} 100,100`}
				...
				{/* Data points - smaller and more subtle */}
